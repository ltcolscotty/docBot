...
def test_get_id_by_name():
    assert (
        googleHandler.get_file_id_by_name("Testing 2: Unpublished", testing_folder)
        == "1CuSugnNmvt39w1YpTdA3lCLyk7KCOim9kOC9uKQsXzY"
    )
...
def test_get_file_link():
    assert (
        googleHandler.get_file_link(testing_folder, "Testing 3: Unpublished")
        == "https://docs.google.com/document/d/1XxDyhaLq2UE84coyKYLvgyDKrgZE9kCocuK-qdBzTYk/edit?usp=drive_link"
    )
...
def test_get_folder_from_docname():
    assert (
        googleHandler.get_folder_from_docname(
            "BM Transparency Report: Fall Quarter 2024"
        )
        == doc_config.folder_id
        or googleHandler.get_folder_from_docname(
            "BM Transparency Report: Fall Quarter 2024"
        )
        == doc_config.share_folder_id
    )
